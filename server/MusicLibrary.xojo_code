...
Protected Module MusicLibrary
	#tag Method, Flags = &h21
		Private Function DecodeSynchsafe(mb As MemoryBlock) As Integer
		  // ID3v2 uses "synchsafe" integers where the 7 least significant bits of each byte are used
	...
	#tag Method, Flags = &h21
		Private Function DecodeText(content As String) As String
		  ...
		  Var text As String = content.Middle(1)
		  Return text.DefineEncoding(GetEncoding(encodingByte))
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function ReadID3Tags(fileData As MemoryBlock) As Dictionary
		  If fileData = Nil Or fileData.Size < 10 Then Return Nil
		  ...
		  If fileData.Size < 10 + tagSize Then Return tags
